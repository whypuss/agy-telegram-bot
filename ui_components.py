...
def format_help_card(current_model: str, timeout_seconds: int) -> str:
    ...
    return (
        ...
        "• `/cancel` — 🛑 中止正在執行的耗時任務\n"
        "• `/clear` — 🧹 清理暫存的多模態快取檔案\n"
        "• `/diff` — 🔍 零 Token 即時檢視工作目錄 Git 代碼變更\n"
        "• `/help` — 📖 顯示此說明卡片\n\n"
        "⚡ **工作流程、推理與自主攻堅：**\n"
        "• `/skills` [名稱] — 🧩 瀏覽與檢視本地或全域技能庫 (Skills)\n"
        "• `/learn` [重點] — 🎓 檢視對話或針對指正提煉硬性規則 (Policies)\n"
        "• `/plan` <需求> — 📋 生成實作方案工件（審閱確認後才動手改代碼）\n"
        "• `/grill-me` <需求> — 🥩 深度面談提問，排查邊界情況與極端情境\n"
        "• `/goal` <目標> — 🎯 持續自主執行、驗證與除錯直到終端目標達成\n"
        "• `/boost` <難題> — 🚀 啟用最高強度深度推理攻克棘手 Bug 或演算法\n"
        "• `/teamwork` <專案> — 👥 調度多 Agent 協同團隊並行推進大規模任務\n"
        "• `/browser` <網址/目標> — 🌐 啟動聯網檢索與網頁內容/UI 渲染驗證\n"
        "• `/btw` <簡短問題> — 💬 背景獨立插問（完全不污染主任務會話記憶）\n"
        "• `/schedule` <時間> <指令> — ⏱️ 設定倒數計時提醒或定時背景任務\n\n"
        "📬 **任務進行中輸入新訊息：**\n"
